...
dashboardControllers.controller('JobOnSiteTechDataNewController', function($scope, $rootScope, $controller, $routeParams, sharedService) {
    ...
    var output = sharedService.getDataFromServer(getJobDetailURL + "/_id/" + $scope._id);
    $scope.jobInfo = output.data[0];
    var output = sharedService.getDataFromServer(getPluralListURL + '/collections/Company~Contact~Region');
	$scope.companyList = output[0].data;
	$scope.contactList = output[1].data;
	$scope.regionList = output[2].data;
	$scope.hphtStandardList = ["HPHT", "Standard"];
	$scope.dayNightList = ["Day", "Night"];
	output = sharedService.getDataFromServer(getOnSiteDailyJobByJobURL + "/_id/" + $scope._id);
    $scope.dailyJob = output.data[0];
	output = sharedService.getDataFromServer(getJarSetsOpenHoleByJobURL + "/_id/" + $scope._id);
	$scope.approval = output.data[0];
    ...
    if ($scope.jobInfo) {
		...
		if ($scope.jobInfo.region) {
			$scope.obj.region = $scope.jobInfo.region;
		}
		if ($scope.jobInfo.field) {
			$scope.obj.field = $scope.jobInfo.field;
		}
		if ($scope.dailyJob.currentRevenue) {
			$scope.obj.currentRevenue = $scope.dailyJob.currentRevenue;
		}
    }
	...
	if ($scope.approval && $scope.approval.itemCount) {
		$scope.obj.summaryCount = $scope.approval.itemCount;
		for (var i = 1; i <= $scope.approval.itemCount; i++) {
			if ($scope.approval['run_' + i] != null) { $scope.obj['run_' + i] = $scope.approval['run_' + i]; }
			if ($scope.approval['toolstringName_' + i] != null) { $scope.obj['toolstring_' + i] = $scope.approval['toolstringName_' + i]; }
			if ($scope.approval['activation_' + i] != null) { $scope.obj['activation_' + i] = $scope.approval['activation_' + i]; }
			if ($scope.approval['toolsFreed_' + i] != null) { $scope.obj['toolsfreed_' + i] = $scope.approval['toolsFreed_' + i]; }
			if ($scope.approval['fishing_' + i] != null) { $scope.obj['fishing_' + i] = $scope.approval['fishing_' + i]; }

			for (var j = 0; j < $scope.toolstringListAll.length; j++) {
				if (($scope.approval['jarSerial_' + i].indexOf(' BA ') != -1 || $scope.approval['jarSerial_' + i].indexOf(' BA') != -1) &&  $scope.toolstringListAll[j].category == 'Baker') { // Baker
					$scope.toolstringList[i].push($scope.toolstringListAll[j]);
				} else if (($scope.approval['jarSerial_' + i].indexOf(' SLB ') != -1 || $scope.approval['jarSerial_' + i].indexOf(' SLB') != -1) &&  $scope.toolstringListAll[j].category == 'Schlumberger') { // Schlumberger
					$scope.toolstringList[i].push($scope.toolstringListAll[j]);
				} else if (($scope.approval['jarSerial_' + i].indexOf(' HAL ') != -1 || $scope.approval['jarSerial_' + i].indexOf(' HAL') != -1) &&  $scope.toolstringListAll[j].category == 'Halliburton') { // Halliburton
					$scope.toolstringList[i].push($scope.toolstringListAll[j]);
				}
			}
    }
	...
	$scope.selectComan = function(item, model, num) {
		$scope.obj['comanEmail_' + num] = null;
		$scope.obj['comanPhone_' + num] = null;
		if (item && item.email) { $scope.obj['comanEmail_' + num] = item.email; }
		if (item && item.phone) { $scope.obj['comanPhone_' + num] = item.phone; }
	};
	$scope.selectEngineer = function(item, model, num) {
		$scope.obj['engineerTitle_' + num] = null;
		$scope.obj['engineerEmail_' + num] = null;
		$scope.obj['engineerPhone_' + num] = null;
		if (item && item.title) { $scope.obj['engineerTitle_' + num] = item.title; }
		if (item && item.email) { $scope.obj['engineerEmail_' + num] = item.email; }
		if (item && item.phone) { $scope.obj['engineerPhone_' + num] = item.phone; }
	};
	$scope.selectGeo = function(item, model, num) {
		$scope.obj['geoTitle_' + num] = null;
		$scope.obj['geoEmail_' + num] = null;
		$scope.obj['geoPhone_' + num] = null;
		if (item && item.title) { $scope.obj['geoTitle_' + num] = item.title; }
		if (item && item.email) { $scope.obj['geoEmail_' + num] = item.email; }
		if (item && item.phone) { $scope.obj['geoPhone_' + num] = item.phone; }
	};
	$scope.selectOther = function(item, model, num) {
		$scope.obj['otherTitle_' + num] = null;
		$scope.obj['otherEmail_' + num] = null;
		$scope.obj['otherPhone_' + num] = null;
		if (item && item.title) { $scope.obj['otherTitle_' + num] = item.title; }
		if (item && item.email) { $scope.obj['otherEmail_' + num] = item.email; }
		if (item && item.phone) { $scope.obj['otherPhone_' + num] = item.phone; }
	};

	// coman list

	$scope.getComanCount = [];
	if (!$scope.obj.comanCount || $scope.obj.comanCount == 0) {
		$scope.obj.comanCount = 3;
	}
	for (var i = 1; i <= $scope.obj.comanCount; i++) {
		$scope.getComanCount.push({id:UUID(), idx:i});
	}

	$scope.addComan = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.comanCount; j > idx ; j--) {
	...
	$scope.deleteComan = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			var idx = arguments[0];

			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.comanCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
				}
			}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.comanCount)];
			}
	...
	$scope.getEngineerCount = [];
	if (!$scope.obj.engineerCount || $scope.obj.engineerCount == 0) {
		$scope.obj.engineerCount = 3;
	}
	for (var i = 1; i <= $scope.obj.engineerCount; i++) {
		$scope.getEngineerCount.push({id:UUID(), idx:i});
	}

	$scope.addEngineer = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.engineerCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteEngineer = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			var idx = arguments[0];

			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.engineerCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
				}
			}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.engineerCount)];
			}
	...
	$scope.getGeoCount = [];
	if (!$scope.obj.geoCount || $scope.obj.geoCount == 0) {
		$scope.obj.geoCount = 3;
	}
	for (var i = 1; i <= $scope.obj.geoCount; i++) {
		$scope.getGeoCount.push({id:UUID(), idx:i});
	}

	$scope.addGeo = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.geoCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteGeo = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			...
			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.geoCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
				}
	}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.geoCount)];
			}
	...
	$scope.getOtherCount = [];
	if (!$scope.obj.otherCount || $scope.obj.otherCount == 0) {
		$scope.obj.otherCount = 3;
	}
	for (var i = 1; i <= $scope.obj.otherCount; i++) {
		$scope.getOtherCount.push({id:UUID(), idx:i});
	}

	$scope.addOther = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.otherCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteOther = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			...
			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.otherCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
    }
    }
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.otherCount)];
    }
	...
	$scope.$on('newCompanyMan',function(event, data) {
    	var output = sharedService.getDataFromServer(getPluralListURL + '/collections/Contact');
    	$scope.contactList = output[0].data;
    	for (var i = 0; i < $scope.contactList.length; i++) {
    		if ($scope.contactList[i]._id == data.value.data._id) {
				$scope.obj['coman_' + data.param] = $scope.contactList[i];
				$scope.selectComan($scope.obj['coman_' + data.param], null, data.param);
    			break;
    		}
    	}
    });
	$scope.$on('newEngineer',function(event, data) {
    	...
    	for (var i = 0; i < $scope.contactList.length; i++) {
    		if ($scope.contactList[i]._id == data.value.data._id) {
				$scope.obj['engineer_' + data.param] = $scope.contactList[i];
				$scope.selectEngineer($scope.obj['engineer_' + data.param], null, data.param);
    			break;
    		}
    	}
    });
	$scope.$on('newGeo',function(event, data) {
    	var output = sharedService.getDataFromServer(getPluralListURL + '/collections/Contact');
    	$scope.contactList = output[0].data;
    	for (var i = 0; i < $scope.contactList.length; i++) {
    		if ($scope.contactList[i]._id == data.value.data._id) {
				$scope.obj['geo_' + data.param] = $scope.contactList[i];
				$scope.selectGeo($scope.obj['geo_' + data.param], null, data.param);
    			break;
    		}
    	}
    });
	$scope.$on('newOther',function(event, data) {
    	var output = sharedService.getDataFromServer(getPluralListURL + '/collections/Contact');
    	$scope.contactList = output[0].data;
    	for (var i = 0; i < $scope.contactList.length; i++) {
    		if ($scope.contactList[i]._id == data.value.data._id) {
				$scope.obj['other_' + data.param] = $scope.contactList[i];
				$scope.selectOther($scope.obj['other_' + data.param], null, data.param);
    			break;
    		}
    	}
    });
	...
	if (!$scope.obj.summaryCount || $scope.obj.summaryCount == 0) {
		$scope.obj.summaryCount = 1;
	}
	for (var i = 1; i <= $scope.obj.summaryCount; i++) {
		$scope.getSummaryCount.push({id:UUID(), idx:i});
	}

	$scope.addSummary = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.summaryCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.getRigMovingCount = [];
	if (!$scope.obj.rigMovingCount || $scope.obj.rigMovingCount == 0) {
		$scope.obj.rigMovingCount = 1;
	}
	for (var i = 1; i <= $scope.obj.rigMovingCount; i++) {
		$scope.getRigMovingCount.push({id:UUID(), idx:i});
	}

	$scope.addRigMoving = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.rigMovingCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteRigMoving = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			var idx = arguments[0];

			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.rigMovingCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
				}
			}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.rigMovingCount)];
			}
	...
	$scope.getRigStatusCount = [];
	if (!$scope.obj.rigStatusCount || $scope.obj.rigStatusCount == 0) {
		$scope.obj.rigStatusCount = 1;
	}
	for (var i = 1; i <= $scope.obj.rigStatusCount; i++) {
		$scope.getRigStatusCount.push({id:UUID(), idx:i});
	}

	$scope.addRigStatus = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.rigStatusCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteRigStatus = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			var idx = arguments[0];

			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.rigStatusCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
				}
			}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.rigStatusCount)];
			}
	...
	$scope.getRigUpcomingCount = [];
	if (!$scope.obj.rigUpcomingCount || $scope.obj.rigUpcomingCount == 0) {
		$scope.obj.rigUpcomingCount = 1;
	}
	for (var i = 1; i <= $scope.obj.rigUpcomingCount; i++) {
		$scope.getRigUpcomingCount.push({id:UUID(), idx:i});
	}

	$scope.addRigUpcoming = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.rigUpcomingCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteRigUpcoming = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			var idx = arguments[0];

			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.rigUpcomingCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
				}
			}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.rigUpcomingCount)];
			}
	...
	$scope.getRigOtherCount = [];
	if (!$scope.obj.rigOtherCount || $scope.obj.rigOtherCount == 0) {
		$scope.obj.rigOtherCount = 1;
	}
	for (var i = 1; i <= $scope.obj.rigOtherCount; i++) {
		$scope.getRigOtherCount.push({id:UUID(), idx:i});
	}

	$scope.addRigOther = function() {
		var idx = arguments[0];
		for (var i = 0; i < arguments.length; i++) {
			for (var j = $scope.obj.rigOtherCount; j > idx ; j--) {
				$scope.obj[arguments[i] + '_' + (j + 1)] = $scope.obj[arguments[i] + '_' + j];
			}
		}
		for (var i = 0; i < arguments.length; i++) {
	...
	$scope.deleteRigOther = function() {
		if (confirm("Are You Sure Want To Delete ?")) {
			var idx = arguments[0];

			for (var i = 0; i < arguments.length; i++) {
				for (var j = idx; j < $scope.obj.rigOtherCount ; j++) {
					$scope.obj[arguments[i] + '_' + (j)] = $scope.obj[arguments[i] + '_' + (j + 1)];
    	}
    		}
			for (var i = 0; i < arguments.length; i++) {
				delete $scope.obj[arguments[i] + '_' + ($scope.obj.rigOtherCount)];
    	}
...
dashboardControllers.controller('JobOnSiteTechDataEditController', function($scope, $rootScope, $controller, $routeParams, sharedService) {
    ...
    if ($scope.jobInfo) {
		...
		if ($scope.jobInfo.region && !$scope.obj.region) {
			$scope.obj.region = $scope.jobInfo.region;
		}
		if ($scope.jobInfo.field && !$scope.obj.field) {
			$scope.obj.field = $scope.jobInfo.field;
		}
		if ($scope.dailyJob.currentRevenue && !$scope.obj.currentRevenue) {
			$scope.obj.currentRevenue = $scope.dailyJob.currentRevenue;
		}
    }
	...
	if ($scope.approval && $scope.approval.itemCount) {
		$scope.obj.summaryCount = $scope.approval.itemCount;
		for (var i = 1; i <= $scope.approval.itemCount; i++) {
			if ($scope.approval['run_' + i] != null && !$scope.obj['run_' + i]) { $scope.obj['run_' + i] = $scope.approval['run_' + i]; }
			if ($scope.approval['toolstringName_' + i] != null && !$scope.obj['toolstring_' + i]) { $scope.obj['toolstring_' + i] = $scope.approval['toolstringName_' + i]; }
			if ($scope.approval['activation_' + i] != null && !$scope.obj['activation_' + i]) { $scope.obj['activation_' + i] = $scope.approval['activation_' + i]; }
			if ($scope.approval['toolsFreed_' + i] != null && !$scope.obj['toolsfreed_' + i]) { $scope.obj['toolsfreed_' + i] = $scope.approval['toolsFreed_' + i]; }
			if ($scope.approval['fishing_' + i] != null && !$scope.obj['fishing_' + i]) { $scope.obj['fishing_' + i] = $scope.approval['fishing_' + i]; }
...
dashboardControllers.controller('JobOnSiteTechDataDetailController', function($scope, $rootScope, $controller, $routeParams, sharedService) {
	...
	$scope.sentEmail = function() {
		...
		var output = sharedService.getDataFromServer(getJarSetsOpenHoleByJobURL + "?_id=" + $scope._id + "&email=" + email);
		alert(output.message);
	};
