    ...
    self.Chat = {
        ...
        addTab: function(roomJid, roomName, roomType) {
            var roomId = Candy.Util.jidToId(roomJid), html = Mustache.to_html(Candy.View.Template.Chat.tab, {
                ...
                isNotLobby: function() {
                	/**
                     * CUSTOM PICO : Lobby room cannot be removed
                     */
                    if (roomJid.indexOf("lobby@") == 0 && roomJid.indexOf("/") == -1) {
                    	return false;
                    } else {
                    	return true;
                    }
...
Candy.View.Template = function(self) {
    ...
    self.Chat = {
        ...
        tab: '<li class="roomtype-{{roomType}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}">' + '<a href="#" class="label">{{#privateUserChat}} {{/privateUserChat}}{{name}}</a>' + '{{#isNotLobby}}<a href="#" class="transition"></a><a href="#" class="close">×</a>{{/isNotLobby}}' + '<small class="unread"></small></li>',
